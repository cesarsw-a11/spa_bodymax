...
export default function RetryGiftCardPaymentButton({ giftCardOrderId }: { giftCardOrderId: number }) {
  const t = useTranslations("giftCardsRetry");
  ...
  async function onRetry() {
    ...
    try {
      const res = await fetch("/api/gift-cards/stripe/create-checkout-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ giftCardOrderId, locale }),
      });
      const json = (await res.json()) as { url?: string; error?: string; errorCode?: string };
      if (!res.ok || !json?.url) {
        throw new Error(resolveApiErrorMessage(json, tApi));
      }
      window.location.href = json.url;
    } catch (err) {
      setErrorMessage(err instanceof Error ? err.message : t("generic"));
    } finally {
      setLoading(false);
    }
  }
