...
export default function FooterContactActions() {
  ...
  const [emailSubject, setEmailSubject] = useState(defaultSubjectFromEnv || t("defaultSubject"));
  ...
  const openEmail = useCallback(() => {
    setContactName("");
    setNameError(false);
    setEmailSubject(defaultSubjectFromEnv || t("defaultSubject"));
    setEmailBody("");
    setCopiedHint(false);
    setOpen(true);
  }, [defaultSubjectFromEnv, t]);
  ...
  const buildEmailPayload = () => {
    ...
    if (!name) return null;
    const subj = emailSubject.trim() || t("defaultSubject");
    const body = emailBody.trim();
    const fullBody = `${t("draftNameLine", { name })}\n\n${body}`;
    const mailto = `mailto:${contactEmail}?subject=${encodeURIComponent(subj)}&body=${encodeURIComponent(fullBody)}`;
    const plainDraft = t("plainMeta", { email: contactEmail, subject: subj, body: fullBody });
  ...
  const copyEmailDraft = async () => {
    ...
    try {
      ...
      setCopiedHint(false);
      window.prompt(t("copyPrompt"), payload.plainDraft);
    }
  };
      ...
      <ul className="mt-5 space-y-3">
        <li>
          <button type="button" onClick={openEmail} className={cardBtn}>
            ...
            <span className="pt-2">
              <span className="block text-sm font-medium text-white">{t("emailLabel")}</span>
              <span className="break-all text-xs text-violet-200/70">{contactEmail}</span>
            </span>
          </button>
        </li>
      </ul>
            ...
            <div className="mt-5 space-y-4">
              <div>
                <label htmlFor="footer-contact-name" className="mb-1.5 block text-xs font-medium text-violet-200/90">
                  {t("yourName")} <span className="text-rose-300/90">{t("required")}</span>
                </label>
                <input
